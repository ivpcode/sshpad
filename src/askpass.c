...
int askpass_init(char *askpass_path_out, int server_port)
{
    ...
    int  slen = snprintf(script, sizeof(script),
        ...
        "        | sed 's/\\(........\\)\\(....\\)\\(....\\)\\(....\\)\\(............\\)/\\1-\\2-\\3-\\4-\\5/')\n"
        "\n"
        "# Codifica percent-encoding completo del prompt.\n"
        "# Ogni byte viene codificato come %%XX (MHD li decodifica automaticamente).\n"
        "# Questo è compatibile con qualsiasi /bin/sh (dash, bash, etc.).\n"
        "encode_prompt() {\n"
